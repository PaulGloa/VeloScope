...
<body>
    <form action="<?php echo base_url('public/Inscription/inscription'); ?>" method="post">
        <div class="connexion-card2">
            <a href="<?php echo base_url('public')?>">
                <img class="logo" src="<?php echo base_url('public/assets/img/logo.jpg');?>">
            </a>
            <p class="title2">Inscription</p>
                <div class="title-name">
                    <p class="str-type3">Nom</p>
                    <p class="str-type3">Prénom</p>
                </div>
                <div class="infos-name">
                    <input type="text" class="info-btn2" placeholder="Nom" name="nom" value="<?= $nom ?>">
                    <input type="text" class="info-btn2" placeholder="Prénom" name="prenom" value="<?= $prenom ?>">
                </div>
            <div class="mail-text">
                <p class="str-type1">Adresse mail</p>
                <input type="text" class="info-btn" placeholder="Adresse mail" name="mail" value="<?= $mail ?>">
            </div>
            <div class="mdp-inscr">
                <p class="str-type1">Mot de passe</p>
                <input type="password" class="info-btn" placeholder="Mot de passe" name="mdp">
                <div class="confirm">
                    <p class="str-type1">Confirmer mot de passe</p>
                    <input type="password" class="info-btn" placeholder="Confirmer mot de passe" name="confirmMdp">
                </div>
            </div>
            ...
            <?php if($erreur != null) : ?>
                <div id="error">
                    <?php if($erreur == 'mail') : ?>
                        <p>Ce mail est déjà enregistré</p>
                    <?php elseif ($erreur == 'mdp') : ?>
                        <p>erreure lors de la confirmation du mot de passe</p>
                    <?php elseif ($erreur == 'mailInvalide') : ?>
                        <p>Adresse mail invalide</p>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
            <p class="piti-txt3">Déjà un compte ? : <a href="<?php echo base_url('public/Connexion'); ?>" class="click-here">Cliquez ici</a></p>
            <input type="submit" value="Inscription" class="seconnecter">
        </div>
    </form>
